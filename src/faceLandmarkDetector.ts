...
import { BlinkingStatus, FaceDetectorScene, IBlinkDetectable } from './FaceDetectorScene'
...
export class Detector
{
	...
	async renderResult()
	{
		...
		if (this.detector != null) 
		{
			...
			try 
				{
				faces = await this.detector?.estimateFaces(Camera.defaultCamera.video, {flipHorizontal: false}) as Array<Face>
				if (faces.length == 1) { this.calculatePosition(faces[0]) }
			} 
				catch (error) 
				{
	...
	calculatePosition(face: Face)
	{
		...
		let rightEyeLidDistance = Math.round(face.keypoints[385].y - face.keypoints[380].y)
		...
		if ( leftEyeLidDistance > -4 && rightEyeLidDistance > -4 ) { newBlink = BlinkingStatus.Both }
		else if ( leftEyeLidDistance < -4 && rightEyeLidDistance >= -4 ) { newBlink = BlinkingStatus.LeftEye }
		else if ( leftEyeLidDistance >= -4 && rightEyeLidDistance < -4 ) { newBlink = BlinkingStatus.RightEye }
		else { newBlink = BlinkingStatus.None }
		...
		if (this.blinkingStatus != newBlink)
		{
			if (this.blinkConfirmationDelay == 10)
			{
				...
				if (FaceDetectorScene.currentScene) { FaceDetectorScene.currentScene.onBlinkStatusChanged(this.blinkingStatus) }
			}
			...
			else { this.blinkConfirmationDelay += 1 }
		}
